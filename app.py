...
print('Looking for pressure sensor streams...')
stream_1 = resolve_stream('name', 'PressureSensor_1')
stream_2 = resolve_stream('name', 'PressureSensor_2')

inlet_1 = None
inlet_2 = None
if stream_1:    
    inlet_1 = StreamInlet(stream_1[0])
    print('Found stream for PressureSensor_1')

if stream_2:
    inlet_2 = StreamInlet(stream_2[0])
    print('Found stream for PressureSensor_2')
    
def new_data(inlet):
    data, timestamp = inlet.pull_sample(0.1, sample=1)
    matrix = np.array(data).reshape(16, 6)

    return matrix.tolist(), timestamp
...
def update_data(n_intervals, store_data1, store_data2):
    global inlet_1, inlet_2

    data1,_ = new_data(inlet_1)
    data2,_ = new_data(inlet_2)
